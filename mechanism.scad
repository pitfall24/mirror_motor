...
module mechanism(tol_boxes = false, axle_tol = 0) {
    ...
    if (tol_boxes) {
        color("orange") translate([21.85, -14.635, 5]) cylinder(d = 2 * hex_ratio + tol_box_tolerance, h = 18 + tol_box_tolerance);
    } else {
        color("gray") translate([21.85, -14.635, 5]) rotate([0, 0, 30]) cylinder(d = 2 * hex_ratio, h = 18, $fn = 6);
    }
    ...
    translate([21.85, -11, 3.556]) {
        ...
        translate([0, -3.635, 6.6]) {
            ...
            if (tol_boxes) {
                color("orange") translate([-11.16, 0, -6 - tol_box_tolerance / 2]) cylinder(d = 2 * hex_ratio + tol_box_tolerance, h = 16 + tol_box_tolerance);
            } else {
    ...
    translate([3.556, -9, 21.85]) {
        ...
        if (tol_boxes) {
            color("orange") translate([0, 6 + tol_box_tolerance / 2, 11.16]) rotate([90, 0, 0]) cylinder(d = 2 * hex_ratio + tol_box_tolerance, h = 12 + tol_box_tolerance);
        } else {
